 ...
 *
 */
public class GroupTzdmCmsProtocol {
	...
	private String buildItemProtocal(ItemList item, int dispScrType){
			Integer delay = item.getDelay() * 10;
			...
			String protocolString = delay + "," + trans + ",0," + trans + "," + speed + ",";
	...
	private String wordParaToString(ItemList item, int dispScrType){
		...
		for(WordList para : list){
			...
			case 0:		
				//字符颜色
				if (item.getFc() == null){
					result += "\\T255255000000";
				}else {
					result += "\\T" + this.hexColor2RGB(item.getFc());
				}
				break;
			//琥珀色
			case 1:	
				result += "\\T000000000255";
				break;
			//无颜色
			case 2:
				break;
			...
			//字间距
			result += "\\M" + this.workspaceTo2SizeString(0);
