...
import cms.impl.GroupTzdmCmsProtocol;
import com.boco.tzdm.constant.ConstVarient;
import com.boco.tzdm.constant.DriverPubConst;
...
import com.boco.tzdm.tcpcomm.RequestEncoder;
import com.boco.tzdm.tcpcomm.ResponseDecoder;
import com.boco.tzdm.constant.DriverConst;
import com.boco.tzdm.tcpcomm.MsgDealHandler;
...
public class TDriver implements IMessageListener, IPortStatusListener {
	 ...
	 * 目标地址
	 */
	private int ftargetAddr;
	/**
	 * 源地址
	 */
	private int foriginAddr;
	/**
	 * 盛放通讯端口对象的线程池
	 */
	...
	public TDriver(TCommPortInfo portInfotemp,RabbitTemplate rabbitTemplatetemp,List<TDriverVarInfo> devDriverVarInfotemp)
    {
		...
		{
			this.ftargetAddr = addrBean.getTargetAddr();
			this.foriginAddr = addrBean.getOriginAddr();
		} else{
			this.ftargetAddr = 0;
			this.foriginAddr = 0;
		}
    ...
    public void AddLoopCheckPkt(){        
        try
        {
            ...
            if (cnt == 0)
            {
				Request Frm = new Request(this.ftargetAddr, this.foriginAddr, DriverConst.Const_Cmd_Check, new byte[]{});
            	Frm.setIndex("-1");
    ...
    public boolean sendData(String businessNo, DevVarInfo devVarInfo)
    {
        ...
        try {
			switch (sendDriverVarInfo.getDwSendCmd()) {
				case DriverConst.Const_Cmd_PlayLst: //播放表
					...
					GroupTzdmCmsProtocol CmsProtocol = new GroupTzdmCmsProtocol();
					String playlist = CmsProtocol.buildProtocal(sendStr);
					System.out.println("下发播放表--->" + playlist);

					ByteArrayOutputStream baos = new ByteArrayOutputStream();
					//设置覆盖
					baos.write("-".getBytes("GBK"));
					//设置文件偏移
					byte[] connArr = new byte[]{0, 0, 0, 0, 0, 0, 0, 0};
					baos.write(connArr);

					//设置播放表名称
					baos.write("play66.lst".getBytes("GBK"));

					baos.write(playlist.getBytes("GBK"));

					ArrSend = baos.toByteArray();
					break;
				case DriverConst.Const_Cmd_ScrOnOff: //开关屏
					if (sendDriverVarInfo.getSzFuncAbbr().equals("OPN")) //开屏
						Dummy = "1";
					else
						Dummy = "0";
					ArrSend = Dummy.getBytes("GBK");
					break;
				case DriverConst.Const_Cmd_ReadCms: //读取屏幕播放表信息
					break;
				case DriverConst.Const_Cmd_Bright: //设备屏幕亮度
					try {
						fValue = Integer.parseInt(sendStr);
						Dummy = CoderUtils.IntTo2SizeString(fValue)
		...
		Frm = new Request(this.ftargetAddr, this.foriginAddr,sendDriverVarInfo.getDwSendCmd(), ArrSend);
	...
	public void onMessageRecv(Object obj) {
		try {
            ...
            if ((Cmd == DriverConst.Const_Cmd_PlayLst)
					|| (Cmd == DriverConst.Const_Cmd_ScrOnOff))
            {
			...
			//应答信息
			switch (Cmd) {
				case DriverConst.Const_Cmd_PlayLst://下发播放表
					if (resp.getDataArray().length == 1 && resp.getDataArray()[0] == DriverConst.Const_Answer_Success) {
						//指定播放表
						byte[] ArrSend = "play66.lst".getBytes("GBK");
						Request req = new Request(this.ftargetAddr, this.foriginAddr, DriverConst.Const_Cmd_DisPlayLst, ArrSend);
						req.setIndex(businessNo);
						List<Request> list = new ArrayList<Request>();
						list.add(req);
						fwindow.addFrame(list);
					} else {
						//发送失败
						RabbitmqCtrlCmdback(businessNo, false);
					}
					break;
				case DriverConst.Const_Cmd_DisPlayLst://播放指定播放表
					if (resp.getDataArray().length == 1 && resp.getDataArray()[0] == DriverConst.Const_Answer_Success) {
						//发送成功
					...
					break;
				//开关屏
				case DriverConst.Const_Cmd_ScrOnOff:
					if (resp.getDataArray().length ==2 && resp.getDataArray()[1] == DriverConst.Const_Answer_Success){
						//发送成功
						RabbitmqCtrlCmdback(businessNo, true);
					} else {
						//发送失败
					...
					break;
				//调节亮度
				case DriverConst.Const_Cmd_Bright:
					if (resp.getDataArray().length == 1 && resp.getDataArray()[0] == DriverConst.Const_Answer_Success) {
						//发送成功
						RabbitmqCtrlCmdback(businessNo, true);
					} else {
					...
					break;
				case DriverConst.Const_Cmd_Check: //返回数据信息
	...
	public void RabbitmqCtrlCmdback(String businessNo,boolean SendFlag) {
		try {
			...
			Identitytemp.setSourceId("collctrsvr_tzdm_cms");
			...
			for (TDriverVarInfo driverVarInfo : this.devDriverVarInfo) {
				...
				if (driverVarInfo.getSzFuncAbbr().indexOf("ER") >= 0){
					try{
						...
						int ERValue = Data[bit - 1] - 0x30;//获取设备状态值
						if (ERValue > 0){
							ERValue=0;
						} else {
							ERValue=1;
    ...
    private boolean getAddressInfo(String addrInfo, AddrInfoBean addrBean)
    {
		...
		// 判断Src中的设备变量属性是否有效
		if (split.length != 2) {
			LOGGER.error("设备地址数量异常");
			return result;
		}

		//进行赋值
		try {
			addrBean.setTargetAddr(Byte.parseByte(split[0]));
			addrBean.setOriginAddr(Byte.parseByte(split[1]));
			result = true;
		} catch (Exception ex) {
...
class AddrInfoBean{
	 ...
	 * 目标地址信息
	 */
	private byte targetAddr;
	/**
	 * 源地址信息
	 */
	private byte originAddr;
	...
	public byte getOriginAddr() {
		return originAddr;
	}

	public void setOriginAddr(byte originAddr) {
